...
export default function ProductsCard() {
  ...
  return (
    <div className="card-scroll-wrapper absolute top-0 left-0 w-full opacity-0 invisible z-20" style={{ height: '200vh' }}>
      ...
      {/* Actual Content */}
      <div 
        className="w-full h-screen bg-[#141414] text-[#E5E5E5] p-4 md:p-8 flex flex-col md:flex-row gap-6 relative shadow-[0_-30px_60px_rgba(0,0,0,0.5)] z-30 overflow-hidden"
        onWheel={handleWheel}
      >
        ...
        <div className="w-full md:w-[45%] h-full flex flex-col justify-between pt-10 pb-4 relative z-30 pointer-events-none">
          ...
          <div className="relative w-full h-[50vh]">
            {products.map((p, idx) => (
              ...
              >
                 <div className="flex flex-col gap-2">
                   <h2 className="font-sans font-light text-[11vw] md:text-[5.5vw] leading-[1.0] tracking-tight uppercase text-[#F2F2F2]" style={{ transform: 'scaleX(1.3)', transformOrigin: 'left' }}>
                     {p.title[0]}
                   </h2>
                   <h2 className="font-sans font-light text-[11vw] md:text-[5.5vw] leading-[1.0] tracking-tight uppercase text-[#F2F2F2]" style={{ transform: 'scaleX(1.3)', transformOrigin: 'left' }}>
                     {p.title[1]}
                 ...
                 <div className="flex justify-start md:ml-[15%] mt-8">
                   <p className="text-xs md:text-sm text-white/50 text-left max-w-[280px] font-light leading-relaxed">
                     {p.desc}
                   </p>
                 </div>
              </div>
            ...
            <div className="absolute bottom-0 left-0 mt-8 flex items-center gap-4 pointer-events-auto">
              <button className="bg-[#A8D0CE] text-black font-bold text-[10px] md:text-[11px] tracking-[0.2em] px-8 py-4 rounded-full uppercase hover:bg-white transition-colors">
                Explore Collection
              </button>
              <button className="w-12 h-12 rounded-full bg-[#A8D0CE] flex items-center justify-center text-black hover:bg-white transition-colors">
          ...
          <div className="w-full h-[30vh] md:h-[32vh] bg-[#D1D1CF] rounded-[3rem] p-4 md:p-6 flex text-[#141414] relative overflow-hidden group shadow-2xl pointer-events-auto">
            ...
            <div className="w-[45%] flex flex-col justify-center items-end text-right z-10 pl-4">
               <p className="text-[10px] md:text-xs font-bold tracking-widest uppercase mb-4 md:mb-6 max-w-[180px] leading-relaxed text-black">
                 ENGINEERED TO DELIVER CLARITY, DEPTH, AND CONTROL.
               </p>
               <button className="border border-black/30 rounded-full px-6 py-2 text-[10px] uppercase tracking-[0.2em] font-semibold hover:bg-[#141414] hover:text-white transition-all duration-300">
                 Discover
               </button>
            </div>
          </div>
        ...
        <div className="hidden md:block w-[55%] h-full relative z-10">
          ...
          <div className="w-full h-full rounded-[3rem] overflow-hidden relative bg-[#1A1A1A] shadow-2xl pointer-events-auto">
             {products.map((p, idx) => (
               <img 
                 key={`hero-${idx}`} 
                 src={p.heroImg} 
          ...
          {/* === TOP RIGHT CUTOUT === */}
          <div className="absolute top-0 right-0 w-24 h-24 bg-[#141414] rounded-bl-[2.5rem] z-20 flex items-center justify-center pt-2 pr-2">
             <button className="w-14 h-14 rounded-full bg-[#A8D0CE] flex items-center justify-center text-black hover:bg-white hover:scale-105 transition-transform duration-300 shadow-xl cursor-pointer pointer-events-auto">
          ...
          <div className="absolute top-[96px] right-0 w-8 h-8 bg-transparent rounded-tr-full shadow-[15px_-15px_0_15px_#141414] z-20 pointer-events-none" />
          <div className="absolute top-0 right-[96px] w-8 h-8 bg-transparent rounded-tr-full shadow-[15px_-15px_0_15px_#141414] z-20 pointer-events-none" />

          {/* === MIDDLE LEFT CUTOUT (Vertical Navigation Pill) === */}
          <div className="absolute top-1/2 -translate-y-1/2 left-0 w-20 h-80 bg-[#141414] rounded-r-[2.5rem] z-20 flex flex-col items-center justify-center pl-2">
             {/* The Pill */}
             <div className="w-14 h-72 bg-[#2A2A2A] rounded-full flex flex-col items-center justify-between py-2 shadow-inner pointer-events-auto border border-white/5">
               {products.map((p, i) => (
                 <div 
                   key={`pill-${i}`} 
                   onClick={() => setActiveIdx(i)} 
                   className={`w-9 h-9 md:w-10 md:h-10 rounded-full overflow-hidden cursor-pointer border-2 transition-all duration-500 ease-out shrink-0 ${i === activeIdx ? 'border-white scale-110 shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'border-transparent opacity-40 hover:opacity-100 hover:scale-105'}`}
                 >
          ...
          <div className="absolute left-0 w-8 h-8 bg-transparent rounded-bl-full shadow-[-15px_15px_0_15px_#141414] z-20 pointer-events-none" style={{ top: 'calc(50% - 192px)' }} />
          <div className="absolute left-0 w-8 h-8 bg-transparent rounded-tl-full shadow-[-15px_-15px_0_15px_#141414] z-20 pointer-events-none" style={{ top: 'calc(50% + 160px)' }} />
